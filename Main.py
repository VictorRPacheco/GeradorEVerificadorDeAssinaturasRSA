...
def RSA_Creator(msg, fileNameCreate):
    print("************ Executando o código de criação do RSA ************")
    # generate a key-pair
    [PublicKey, PrivateKey] = gerandoChaves()

    print(f"Public key:  (n={hex(PublicKey[0])}, e={hex(PublicKey[1])})")
    print(f"Private key: (phiN={hex(PrivateKey[0])}, d={hex(PrivateKey[1])}")
    ...
    # 3) Save the result in a pkl file
    Data = {
...
def RSA_Reader(fileNameRead):
    print("************ Executando o código de decifração do RSA ************")
    # Parsing of the signed document
    print(f"STATUS: Lendo o arquivo {fileNameRead}")
    ...
    print(f"STATUS: Arquivo lido com sucesso {fileNameRead}")
